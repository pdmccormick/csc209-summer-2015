 ...
 * results follow. Go back and forth between your servers in any order, and the
 * client will receive each of them correctly.
 */
int main(int argc, char *argv[])
{
    ...
    while (fds[0] != -1 || fds[1] != -1) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            // Find maximum numeric FD
            if (fds[i] > max_fd) {
                max_fd = fds[i];
            }
        }

        // Select on any available-for-reading FD's
